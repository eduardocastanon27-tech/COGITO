...
description: Full metacognitive protocol for substantial multi-step or multi-session work — externalized state files, explicit assumptions, honest solve-vs-help decisions, end-of-task §7 finish-line review. The always-on core (assume out loud, verify before claiming done, lesson write-back) is ALREADY injected by the SessionStart hook — do not load this skill just for that. Invoke it for genuinely large work, or on "Cogito", "checkpoint", "extended mind", "remember this for next time".
---
...
**Wins ledger — the strategy playbook (2026-07-03, ACE).** Scars are half the memory. The other half is `PLAYBOOK.md` (next to the ledger): imperative strategies that visibly WORKED, as bullets `- [P###][#tag][helpful:N][harmful:N] strategy {via:origin}`. Capture prospectively, same as scars: when an approach visibly works, append a bullet (fresh id, zeroed counters); when a loaded bullet helps, increment its `[helpful:]` in place; when one misleads, increment `[harmful:]`. Delta-only — never rewrite the playbook wholesale (ACE: monolithic rewrites cause context collapse). Curation (dedup, merge, demote harmful>helpful to PLAYBOOK-ARCHIVE.md) happens only at consolidation time. The counters replace the old `[seen:]` stamp as the primary evidence-of-use signal. Research: ACE (arXiv 2510.04618) — strategy accumulation with counters is what lifted a smaller model to top-agent level; scars alone only prevent repeats, strategies compound wins.
